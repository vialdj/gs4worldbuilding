...
class TerrestrialWorld(model.RandomizableModel):
    """the World Model"""
...
threshold in K"""
    ...
    def random_resource(self):
        """sum of a 3d6 roll times over default worlds Resource Value Table"""
        resource_dist = [0, 0, 0, .01852, .14352, .67592,
                         .14352, .01852, 0, 0, 0]
...
the same type"""
    ...
    def density_bounds(self) -> model.bounds.QuantityBounds:
        ...
        return (model.bounds.QuantityBounds(*self.core.value)
    ...
    @property
    def hydrographic_coverage_bounds(self) -> model.bounds.ValueBounds:
    ...
    def randomize(self):
        ...
        super(TerrestrialWorld, self).randomize()
    ...
    def __init__(self, orbit=None):
        ...
        if orbit:
            if not orbit._body:
                orbit._body = self
            world = self
            world.__class__ = (moon(type(self))
                               if issubclass(type(orbit._parent_body),
                                             type(self))
                               else planet(type(self)))
...
def planet(world):
    ...
    class TerrestrialPlanet(world):
        """the orbiting world extended model"""
        _precedence = [p for p in TerrestrialWorld._precedence
                       if p != 'temperature']
